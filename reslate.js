...
if (process.argv[2] === 'init') {
  ...
  cp.spawn('npm',[ 'run', 'build.local' ], { stdio: 'inherit', cwd: __dirname, env: env }, function(err, output) {
    if (err) console.warn(err);
    console.log(output);
  });
}
else if (process.argv[2] === 'serve') {
  cp.spawn('npm',[ 'run', 'serve.local' ], { stdio: 'inherit', cwd: __dirname, env: env }, function(err, output) {
    if (err) console.warn(err);
    console.log(output);
  });
}
